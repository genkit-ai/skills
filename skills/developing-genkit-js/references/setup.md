...
8. Add a `genkit:dev` script to `package.json` that runs `genkit start -- npx tsx --watch {sourceDir}/ai/index.ts` (use `npx genkit-cli`, or `pnpm dlx` / `yarn dlx` for those package managers, if the CLI is not installed globally). This is the recommended way to run the app during development: `genkit start` wraps your program and captures a trace of every Genkit action (prompts, model I/O, tool calls) while serving the Dev UI at http://localhost:4000. Running the app directly (`tsx`/`node`/`npm start`) skips trace capture, so you're debugging blind. DO NOT try to run the script now.
