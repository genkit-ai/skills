...
### Model References
-   **Correct (v1.x)**: Use plugin-specific model factories or string identifiers.
    ```ts
    await ai.generate({ model: googleAI.model('gemini-2.5-flash'), ... });
    ```
-   **Incorrect (Pre-1.0)**: Using imported model objects directly without plugin context.
    ```ts
...
-   **Missing responseModalities**: When using image generation models (like `gemini-2.5-flash-image-preview`), you **MUST** specify the response modalities in the config.
    ```ts
